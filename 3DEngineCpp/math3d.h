...
public:
	...
	inline void InitRotateTransform(T rotateX, T rotateY, T rotateZ)
	{
		...
		rx[1][0] = T(0); rx[1][1] = cos(x); rx[1][2] = -sin(x); rx[1][3] = T(0);
		rx[2][0] = T(0); rx[2][1] = sin(x); rx[2][2] = cos(x) ; rx[2][3] = T(0);
		rx[3][0] = T(0); rx[3][1] = T(0)  ; rx[3][2] = T(0)   ; rx[3][3] = T(1);

		ry[0][0] = cos(y); ry[0][1] = T(0); ry[0][2] = -sin(y); ry[0][3] = T(0);
		ry[1][0] = T(0)  ; ry[1][1] = T(1); ry[1][2] = T(0)   ; ry[1][3] = T(0);
		ry[2][0] = sin(y); ry[2][1] = T(0); ry[2][2] = cos(y) ; ry[2][3] = T(0);
		ry[3][0] = T(0)  ; ry[3][1] = T(0); ry[3][2] = T(0)   ; ry[3][3] = T(1);

		rz[0][0] = cos(z); rz[0][1] = -sin(z); rz[0][2] = T(0); rz[0][3] = T(0);
		rz[1][0] = sin(z); rz[1][1] = cos(z) ; rz[1][2] = T(0); rz[1][3] = T(0);
	...
	inline void InitCameraTransform(const Vector3<T>& target, const Vector3<T>& up)
	{
		...
		(*this)[0][0] = u.GetX();   (*this)[0][1] = u.GetY();   (*this)[0][2] = u.GetZ();   (*this)[0][3] = T(0);
		(*this)[1][0] = v.GetX();   (*this)[1][1] = v.GetY();   (*this)[1][2] = v.GetZ();   (*this)[1][3] = T(0);
		(*this)[2][0] = n.GetX();   (*this)[2][1] = n.GetY();   (*this)[2][2] = n.GetZ();   (*this)[2][3] = T(0);
	...
	inline void InitPersProjTransform(T fov, T aspectRatio, T zNear, T zFar)
	{
		...
		(*this)[2][0] = T(0);                   (*this)[2][1] = T(0);            (*this)[2][2] = (-zNear - zFar)/zRange ; (*this)[2][3] = T(2)*zFar*zNear/zRange;
		(*this)[3][0] = T(0);                   (*this)[3][1] = T(0);            (*this)[3][2] = T(1);            (*this)[3][3] = T(0); 
	}
protected:
private:
};
